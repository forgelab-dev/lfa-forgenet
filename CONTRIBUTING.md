...
Utilisez le signalement privé de GitHub (onglet **Security** → **Report a vulnerability**) ou, sans compte GitHub, écrivez à **contact@lucidforgeafrica.com** avec « Sécurité ForgeNet » en objet, sans détail technique dans ce premier message. Nous accusons réception sous 72 heures ouvrées. Le processus complet (délais, périmètre, divulgation coordonnée) est décrit dans [SECURITY.md](SECURITY.md). ForgeNet gère des secrets, des accès serveur et des données de production : ce canal est pris au sérieux.
